...
export default function Home(props) {
  ...
  return (
    <>
      ...
      <Modal
        visible={false}
        closeModalHandler={() => dispatch(setWelcomePopup(false))}
        style={{ margin: "0 20px" }}
      >
        <div className="position-relative">
          <button
            className="iiz__btn iiz__close iiz__close--visible"
            onClick={() => dispatch(setWelcomePopup(false))}
          ></button>
            ...
            <Link href="/products?type=masher-bazaar&name=Masher Bazaar&filter=paginate_no__40">
              <a onClick={() => dispatch(setWelcomePopup(false))}>
                <img
                  ...
                  src="/images/campaign/masher-bazaar-welcome.jpg"
                  alt="super sale popup"
                />
              </a>
            </Link>
          </div>
      ...
      {/* <ProductSection title={translate('Daily Essential')} type="daily-essentials" limit={isMobile ? 6 : 10} url='daily-essentials' isSliding={false} /> */}
      {/* <ProductSection title={translate('Fastest Delivery')} type="fastest" limit={6} url='fastest' /> */}
      <ProductSection
        title={translate("Latest Products")}
        type="latest"
        limit={18}
        url="latest"
      />
...
export const getServerSideProps = async () => {
  try {
    const [sliderRes, categoryRes] = await Promise.all([
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}frontend-categories?type=homepage&limit=12`
      ),
    ]);
    ...
    const homepageCategories = homepageCategoriesObj.status
      ? homepageCategoriesObj.data
      : [];
